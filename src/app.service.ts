...
import { AdmLayer, AdmLayerType, Location, Prisma, UserEnteredAddress } from '@prisma/client';
...
export class AppService {
	...
	private readonly latLongRegex = /^-?([1-8]?\d(\.\d+)?|90(\.0+)?),\s*-?(1[0-7]\d(\.\d+)?|180(\.0+)?|[1-9]?\d(\.\d+)?)$/;

	async location_with_filter(filter: Prisma.LocationWhereInput): Promise<LocationWithDetails[]> {
	...
	async getOSMData(text_input: string): Promise<ResponseData[] | []> {
	...
	async getOSMDataCoordinates(coordinates: string[]): Promise<ResponseData[] | []> {
		let data = qs.stringify({
			'api_key': process.env.GEOCODE_KEYS,
			'layers': 'coarse',
	...
	get_osm_response(locations: any) {
	...
	async get_response(locations: LocationWithDetails[]): Promise<ResponseData[]> {
		...
		locations.map((location) => {
			...
			if (address_details.street_address == "NA" && address_details.locality == "NA" && address_details.landmark == "NA") {
		...
		if (Object.keys(missing_data_coordinates).length > 0) {
			...
			Object.keys(missing_data_coordinates).map((key) => {
				promises.push(this.getOSMDataCoordinates([missing_data_coordinates[key].long, missing_data_coordinates[key].lat]))
			})
			...
			all_missing_results.map((result, i) => {
				if (result.status == "fulfilled") {
					...
					const missing_value = this.get_osm_response(result.value)
	...
	async getGeocoding(text_input: string): Promise<ResponseData[] | null> {
		...
		if (this.latLongRegex.test(text_input)) {
			const coordinates_array = text_input.split(",").map((cord) => cord.trim())
			locations = await this.get_response(await this.location_using_location(coordinates_array))
			if (locations.length === 0) {
				locations = this.get_osm_response(await this.getOSMDataCoordinates(coordinates_array))
			}
		} else {
			locations = await this.get_response(await this.location_using_text(text_input))
			if (locations.length === 0) {
				locations = this.get_osm_response(await this.getOSMData(text_input))
			}
		}
		return locations
	}
